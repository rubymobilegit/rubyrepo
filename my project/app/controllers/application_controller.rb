class ApplicationController < ActionController::Base
  ...
  def index
    ...
    if /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.match(agent)
      #redirect_to google_search_path and return
    end
    @browser = :unknown
    case agent.browser
    ...
    # zip_code = (current_user.present? && !current_user.zip_code.blank?) ? current_user.zip_code : default_zip_code
    # ctc_keywords = ["Plumbers", "HVAC", "Electricans", "Insurance", "Cable/Dish TV", "Credit Repair"]
    # ctc_keywords.each do |keyword|
    ...
    #   end
    #   @ctc_merchants.merge!(keyword => search_soleo_and_local_merchants_db(search, search_intent, zip_code))
    # end
