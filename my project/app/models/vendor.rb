class Vendor < ActiveRecord::Base
  ...
  has_many :auctions, :through=>:bids, 
    :select=>'auctions.*, bids.current_value as vendor_bid, bids.is_winning as vendor_won,  bids.max_value as vendor_max_bid'
  has_many :won_auctions, :through=>:bids, :source=>:auction, :conditions=>{:bids=>{:is_winning=>true}}, 
    :select=>'auctions.*, bids.current_value as vendor_bid, bids.is_winning as vendor_won,  bids.max_value as vendor_max_bid'
  has_many :campaigns, :dependent=>:destroy
  has_many :offers, :dependent=>:destroy
  has_many :offer_images, :dependent=>:destroy, :conditions => "offer_id is null"
  has_many :all_offer_images, :class_name=>'OfferImage'
  has_many :funds_transfers, :dependent=>:restrict
  has_many :funds_refunds, :dependent=>:restrict
  has_one :vendor_setting, :dependent => :destroy
  has_and_belongs_to_many :service_categories
  has_and_belongs_to_many :product_categories
  has_many :keywords, :class_name=>'VendorKeyword', :dependent=>:destroy, :order=>'vendor_keywords.keyword ASC'
  ...
  has_many :transactions, :class_name=>'VendorTransaction', :dependent=>:delete_all, :order=>'vendor_transactions.created_at, vendor_transactions.id ASC'
  ...
  validates :zip_code, :presence => true, :format => { :with => /^\d{5}$/i }, :unless=>Proc.new{create_by_admin}
