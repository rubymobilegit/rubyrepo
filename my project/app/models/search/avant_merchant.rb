class Search::AvantMerchant < Search::Merchant
  has_one :commission, :class_name => 'AvantCommission'
  belongs_to :advertiser, :class_name => 'AvantAdvertiser', :foreign_key => 'db_id'
  ...
  def set_attributes_from_search(advertiser, offer, controller)
    ...
    add_intent_id_to_trackable_url('ctc')


    self.company_coupons_url = controller.auction_coupons_url('avant', advertiser.advertiser_id)
  end
end
