class Search::Intent < ActiveRecord::Base
  ...
  def send_soleo_callback
    Delayed::Worker.logger.info '********************************************* ' + Time.now.in_time_zone('Eastern Time (US & Canada)').strftime('%m-%d-%y %I-%M-%S %p') + ' *********************************************'
    ...
    if soleo_merchants.length > 0
      ...
      Delayed::Worker.logger.info '******* ' + Time.now.in_time_zone('Eastern Time (US & Canada)').strftime('%m-%d-%y %I-%M-%S %p') + ': Soleo callback request:'
      Delayed::Worker.logger.info '******* '+ builder.to_xml.to_s

      response = callback_request.run
      Delayed::Worker.logger.info '******* ' + Time.now.in_time_zone('Eastern Time (US & Canada)').strftime('%m-%d-%y %I-%M-%S %p') + ': Soleo callback response:'
      ...
      Delayed::Worker.logger.info '******* ' + Time.now.in_time_zone('Eastern Time (US & Canada)').strftime('%m-%d-%y %I-%M-%S %p') + ': No merchants for soleo callback'
    end
