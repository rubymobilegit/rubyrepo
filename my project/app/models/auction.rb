class Auction < ActiveRecord::Base
  ...
  has_many :bids, :dependent=>:destroy, :order=>'bids.max_value DESC, bids.updated_at ASC, bids.id ASC '
  has_many :winning_bids, :class_name=>'Bid', :order=>'bids.max_value DESC, bids.updated_at ASC, bids.id ASC ', :conditions=>{:is_winning=>true}
  has_one :highest_bid, :class_name=>'Bid', :order=>'bids.max_value DESC'
  has_one :lowest_bid, :class_name=>'Bid', :order=>'bids.max_value ASC'
  has_many :archived_bids, :dependent=>:destroy
  has_many :participants, :source=>:vendor, :through=>:bids
  has_many :winners, :through=>:bids, :source=>:vendor, :conditions=>{:bids=>{:is_winning=>true}}
  ...
  has_many :offers, :through=>:bids, :conditions=>{:bids=>{:is_winning=>true}}
  ...
  def autobid_campaigns
    ...
    order_by_max_bids_desc_and_shuffle_same_max_bids(applicable_campagins_with_enugh_funds).each do |campaign|
      ...
      Bid.update_all({:updated_at=>Time.now+1.second}, {:id=>manual_bids_winning.map{|b| b.id}})
      bids.reload
    end
