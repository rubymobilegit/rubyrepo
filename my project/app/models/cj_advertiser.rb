class CjAdvertiser < ActiveRecord::Base
  ...
  def self.reload_all_advertisers
    ...
    begin
      ...
      self.transaction do
        advertisers.each do |row|
          ...
          advertiser ||= self.find_or_initialize_by_advertiser_id row['advertiser_id']
    ...
    self.update_all({:inactive => true}, ['sample_link_id is null'])
    inserted_count
    ...
    advertisers.each do |row|
      ...
      sample_link_request.on_complete do |link_response|
        if link_response.success?
          ...
          row['sample_link_id'] = cj_result['links']['link']['link_id'] if (cj_result && cj_result['links'] && cj_result['links']['link'] && cj_result['links']['link']['link_id'] && cj_result['links']['link']['advertiser_id'])
        end
      end
